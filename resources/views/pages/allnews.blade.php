@extends('layouts.app')
@section('title', 'All news | ' . config('app.name'))
...
@section('maincontent')
    ...
    {{-- BOTÃO DE FOLLOW DA CATEGORIA - Aparece apenas quando estamos vendo uma categoria específica --}}
    @isset($category)
        <div style="display: flex; align-items: center; justify-content: space-between; max-width: 890px; margin: 0 auto 20px auto; padding: 15px; background: #f5f5f5; border-radius: 5px; border: 1px solid #ddd;">
            <div>
                <h2 style="margin: 0; font-size: 1.5rem;">{{ $category->name }}</h2>
                <small style="color: #666;">{{ $category->followers()->count() }} followers</small>
            </div>

            @auth
                <button
                    id="category-follow-btn"
                    class="button {{ $isFollowingCategory ? 'button-unfollow' : 'button-follow' }}"
                    data-following="{{ $isFollowingCategory ? '1' : '0' }}"
                    data-follow-url="{{ route('category.follow', $category) }}"
                    data-unfollow-url="{{ route('category.unfollow', $category) }}"
                    style="padding: 10px 20px;"
                >
                    {{ $isFollowingCategory ? 'Unfollow' : 'Follow' }}
                </button>
            @endauth
        </div>
    @endisset
    ...
    <div id="news-container" style="position: relative; z-index: 1; min-height: 600px;">
        @if(empty($news_categories))
            <p style="text-align: center; color: #666; margin-top: 50px;">No news found matching your criteria.</p>
        @else
